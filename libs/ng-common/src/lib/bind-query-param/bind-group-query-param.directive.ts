...
export class BindGroupQueryParamDirective implements OnInit, OnDestroy {
  ...
  private logger = new Logger('BindGroupQueryParamDirective');
  ...
  set _config(value: Partial<BindQueryParamOptions>) {
    ...
    if (value.logger) {
      this.logger.setConfig(value.logger);
    }
  }
  ...
  private checkChange(
    ...
    if (params.value != params.current) {
      ...
      this.logger.verbose({
        action: params.action,
        message: `change detected for ${params.key}: ${
          params.current
      ...
      this.logger.verbose({
        action: params.action,
        message: `ignore unchanged ${params.key}:${params.value}`,
      });
    }
  }
}
